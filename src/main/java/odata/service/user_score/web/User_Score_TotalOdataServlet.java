...
public class User_Score_TotalOdataServlet extends HttpServlet {

  private static final long serialVersionUID = 1L;
  private static final Logger LOG = LoggerFactory.getLogger(User_Score_TotalOdataServlet.class);
  ...
  protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    try {
      ...
      Storage storage = (Storage) session.getAttribute(Storage.class.getName() + "all");
      if (storage == null) {
    	  //JWTFactory.getJWT_USER((String)session.getAttribute(JWTFactory.session_name));
        //storage = new Storage(JWTFactory.getJWT_USER((String)session.getAttribute(JWTFactory.session_name)),true);
    	  storage = new Storage(JWTFactory.getJWT_USER((String)req.getHeader(JWTFactory.session_name)),true);
      	
    	  //storage = new Storage("tom");
        session.setAttribute(Storage.class.getName() + "all", storage);
      }
      //storage.initData();
